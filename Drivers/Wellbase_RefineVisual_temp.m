...
% T_fovData, T_fovInfo, stim
T_fovData = evalout.T_fovData;
T_fovInfo = evalout.T_fovInfo;
stim = evalout.stim;
...
for i = 1:n_fov
    s = sig.arbitrary();
    if isempty(T_fovData{i, 'trace'}{1})
        traceEmpty = [traceEmpty,i];
        s.setSignal(T_fov{i, 'trace_t'}{1}, ones(size(T_fov{i, 'trace_t'}{1})));
    elseif iscell(T_fovData{i, 'trace'}{1})
        s.setSignal(T_fov{i, 'trace_t'}{1}, cell2mat(T_fovData{i, 'trace'}{1}));
    else
...
T_fov(traceEmpty,:) = [];
T_fovInfo(traceEmpty,:) = [];
T_fovData(traceEmpty,:) = [];
n_fov = height(T_fovData);
%% Aligmment and Detrend (You need to only run the section according to protocol)
% align using template 1p
parseProtocol = split(stimname,' ');
Protocol = char(parseProtocol(1));
switch Protocol
    case char('sync')
...
switch Protocol
    case(char('2p'))
        % Event averaging
        traces = sig.arbitrary.empty();
        sevent_t = seventRange.t_on(1:20) - 0.1;
        for i = 1:n_fov
...
save('D:\OneDrive - rice.edu\Francois\Paper\JEDI-2P\Figures\BenchmarkingPlots\20190227_benchmarking\20190227_benchmarking_plate1\20190227 Fstim platform benchmarking 20x plate1 1khzFstim nonTriggered_20200410112943_quant.mat', 'stim', 'T_fov', 'T_fovInfo', 'T_well', 'T_groupFromFOV', 'T_groupFromWell');
