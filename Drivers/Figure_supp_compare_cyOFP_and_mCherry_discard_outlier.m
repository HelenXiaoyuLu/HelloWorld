...
% Load local
dirp = 'D:\OneDrive - Baylor College of Medicine\Paper_201906_GEVI\Fstim\20200814 benchmarking cyOFP and mCherry';
j1.output = load(fullfile(dirp, '1pFstim 20200814 20201020161717')); % 1p Field stimulation
j2.output = load(fullfile(dirp, 'Traditional1Pbleaching 20200814 20201002192033'));    % 1p Photobleaching
j.output = load(fullfile(dirp, '2pFstim 20200814 20200929223023 refCh2')); % 2p Field stimulation
j1.input = '\\stpierrelab7910\E\Image\Xiaoyu\20201014 Evolution path';
j.input = '\\stpierrelab7910\E\Image\Xiaoyu\20201014 Evolution path';
...
T_group.("-dF/F0 Short STD") = splitapply(@std, ...
    T_plot_filtered.("-dF/F0 Short Mean"), T_plot_filtered.Area, grouping);
T_group.("-dF/F0 Long Mean") = splitapply(weightedMean, ...
    T_plot_filtered.("-dF/F0 Long Mean"), T_plot_filtered.Area, grouping);
T_group.("-dF/F0 Long STD") = splitapply(@std, ...
    T_plot_filtered.("-dF/F0 Long Mean"), T_plot_filtered.Area, grouping);
...
T_group.("Photostability STD") = splitapply(@std, ...
    T_plot_filtered.("Photostability Mean"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index Short") = splitapply(weightedMean, ...
    T_plot_filtered.("Detectability Index Short"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index Short STD") = splitapply(@std, ...
    T_plot_filtered.("Detectability Index Short"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index Long") = splitapply(weightedMean, ...
    T_plot_filtered.("Detectability Index Long"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index Long STD") = splitapply(@std, ...
    T_plot_filtered.("Detectability Index Long"), T_plot_filtered.Area, grouping);
...
T_group.Area = splitapply(@sum, T_plot_filtered.Area, grouping);
T_group.("Brightness Mean") = splitapply(weightedMean, ...
    T_plot_filtered.("Relative brightness (GFP/RFP) Mean"), T_plot_filtered.Area, grouping);
T_group.("Brightness STD") = splitapply(@std, ...
    T_plot_filtered.("Relative brightness (GFP/RFP) Mean"), T_plot_filtered.Area, grouping);

T_group.("-dFF0 Stim    1ms 60V Mean") = splitapply(weightedMean, ...
    T_plot_filtered.("-dFF0 Stim    1ms 60V"), T_plot_filtered.Area, grouping);
T_group.("-dFF0 Stim    1ms 60V STD") = splitapply(@std, ...
    T_plot_filtered.("-dFF0 Stim    1ms 60V"), T_plot_filtered.Area, grouping);
T_group.("-dFF0 Stim  1.5ms 20V Mean") = splitapply(weightedMean, ...
    T_plot_filtered.("-dFF0 Stim  1.5ms 20V"), T_plot_filtered.Area, grouping);
T_group.("-dFF0 Stim  1.5ms 20V STD") = splitapply(@std, ...
    T_plot_filtered.("-dFF0 Stim  1.5ms 20V"), T_plot_filtered.Area, grouping);
T_group.("-dFF0 Stim 92.5ms 30V Mean") = splitapply(weightedMean, ...
    T_plot_filtered.("-dFF0 Stim 92.5ms 30V"), T_plot_filtered.Area, grouping);
T_group.("-dFF0 Stim 92.5ms 30V STD") = splitapply(@std, ...
    T_plot_filtered.("-dFF0 Stim 92.5ms 30V"), T_plot_filtered.Area, grouping);

T_group.("Detectability Index   1ms 60V") = splitapply(weightedMean, ...
    T_plot_filtered.("Detectability Index   1ms 60V"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index   1ms 60V STD") = splitapply(@std, ...
    T_plot_filtered.("Detectability Index   1ms 60V"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index 1.5ms 20V") = splitapply(weightedMean, ...
    T_plot_filtered.("Detectability Index 1.5ms 20V"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index 1.5ms 20V STD") = splitapply(@std, ...
    T_plot_filtered.("Detectability Index 1.5ms 20V"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index92.5ms 30V") = splitapply(weightedMean, ...
    T_plot_filtered.("Detectability Index92.5ms 30V"), T_plot_filtered.Area, grouping);
T_group.("Detectability Index92.5ms 30V STD") = splitapply(@std, ...
    T_plot_filtered.("Detectability Index92.5ms 30V"), T_plot_filtered.Area, grouping);
...
%% Figure 1: compare dF/F0
ppmean = "-dFF0 Stim 92.5ms 30V Mean";
ppstd = "-dFF0 Stim 92.5ms 30V STD";
T_plot_mCherry = sortrows(T_plot_mCherry, ppmean, 'ascend');
T_plot_cyOFP = T_plot_cyOFP(T_plot_mCherry.GEVI,:);
x = T_plot_mCherry.(ppmean);
y = T_plot_cyOFP.(ppmean);
...
%% Figure 3: compare photostability
ppmean = "Brightness Mean";
ppstd = "Brightness STD";
T_plot_mCherry = sortrows(T_plot_mCherry, ppmean, 'ascend');
T_plot_cyOFP = T_plot_cyOFP(T_plot_mCherry.GEVI,:);
x = T_plot_mCherry.(ppmean);
y = T_plot_cyOFP.(ppmean);
...
%% Figure 5: Scatter_rand2dwell
pp1 = "-dFF0 Stim 92.5ms 30V";
pp2 = "Relative brightness (GFP/RFP) Mean";
cmap = hsv(9);
f = figure('Color', [1, 1, 1]);
f.UserData.PlotData1 = T_plot_cyOFP_well;   % bind data
f.UserData.PlotData2 = T_plot_mCherry_well;
