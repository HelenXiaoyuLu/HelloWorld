...
saveName = '20190720 XLRED_001-002_pipeval';
...
parfor i = 1:n_fov
    ...
    y3 = traceIn.crop(seventRange.t_on(7)-0.2,seventRange.t_on(7)+0.5).y;
    x3 = traceIn.crop(seventRange.t_on(7)-0.2,seventRange.t_on(7)+0.5).x;
    y4 = traceIn.crop(seventRange.t_on(8)-0.2,seventRange.t_on(8)+0.5).y;
    x4 = traceIn.crop(seventRange.t_on(8)-0.2,seventRange.t_on(8)+0.5).x;
    y5 = traceIn.crop(seventRange.t_on(9)-0.2,seventRange.t_on(9)+0.5).y;
    x5 = traceIn.crop(seventRange.t_on(9)-0.2,seventRange.t_on(9)+0.5).x;
    s = sig.arbitrary();
    y = [y1;y2;y3;y4;y5];
    x = [x1;x2;x3;x4;x5];%0.3+(0:length(y)-1)*0.001;
    s.setSignal(x, y);
    traces(i, 1) = s;
end
T_fov.TraceFinal = traces;
% seventRangetrim = seventRange;
% seventRangetrim.t_on = [0.5;1.0;1.5;2.1;2.7];
% seventRangetrim.t_preon = [0.5;1.0;1.5;2.1;2.7];
% seventRangetrim.t_off = [0.5;1.0;1.5;2.1;2.7];
% seventRangetrim.t_preoff = [0.5;1.0;1.5;2.1;2.7];
% seventRangetrim.V_pulse = ones(5,1);
% seventRangetrim.n_pulse = 5;
...
% Tests
plot(traces(2).x,mean([traces(1).y,traces(2).y,traces(3).y,traces(4).y,traces(5).y,traces(6).y],2))
figure()
hold on
for i = 1:6
    subplot(2,3,i)
    plot(traces(i).x, traces(i).y)
end
...
T_well.wellName = splitapply(@unique, T_fovInfo.wellName, FOVgroups);
...
for i = 1:n_fov
    ...
    delay = T_fov.Delay(i);
%     RMean(i) = T_fov.TraceRaw(i).integral(0 - delay, 0.01 - delay)./0.01;
    RMean(i) = T_fov.TraceRaw(i).integral(0, 0.01)./0.01;
    BMean(i) = mean(BMap(finalMask));
end
T_fov.RMean = RMean;
T_fov.BMean = BMean;
T_fov.RBRatio = T_fov.RMean./T_fov.BMean;
...
FOVgroups = findgroups(T_fov.CoordWell(:, 1), T_fov.CoordWell(:, 2));
% mean 
T_well.RBRatioMean = splitapply(@mean, T_fov.RBRatio, FOVgroups);
T_well.AOCMean = splitapply(@mean, T_fov.AOC, FOVgroups);
% std
T_well.RBRatioStd = splitapply(@std, T_fov.RBRatio, FOVgroups);
T_well.AOCStd = splitapply(@std, T_fov.AOC, FOVgroups);
% SEM
T_well.RBRatioSEM = splitapply(@(x) std(x)./sqrt(numel(x)), T_fov.RBRatio, FOVgroups);
...
T_groupFromFOV;
T_groupFromWell;
savedir = 'D:\OneDrive - rice.edu\Francois\RedGEVIs\Wet\Results\New pipeline since 20200430\quant\';
save(strcat(savedir,saveName,'_quant.mat'), 'stim', 'T_fov', 'T_fovInfo', 'T_well', 'T_groupFromFOV', 'T_groupFromWell');
