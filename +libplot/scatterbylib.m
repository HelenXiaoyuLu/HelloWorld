...
function [libTable,f1,c1,d1,s1] = scatterbylib(libTable, pp1, pp2, varargin)
    %% parse input
    c = colormap(lines);
    p = inputParser;
    ...
    switch doNorm
        case 0
            doScatter(libTable,pp1,pp2,pp3,controls,selectLib,figshow,cmap,norm2ctrl,datatip);
        case 1
            ...
            meanctrlpp2 = nanmean(Tnormctrl{:,pp2col});
            libTable.normpp2 = libTable{:,pp2col}/meanctrlpp2;
            newpp2 = strcat(pp2,'Normalized to',norm2ctrl{1});
            libTable.Properties.VariableNames{end} = newpp2;
            [f1,c1,d1,s1] = doScatter(libTable,newpp1,newpp2,pp3,controls,selectLib,figshow,cmap,norm2ctrl,datatip); 
    end
end

function [f,c,d,s] = doScatter(libTable,pp1,pp2,pp3,controls,selectLib,figshow,cmap,norm2ctrl,datatip)
    ...
    for l = 1:length(libsel)
        ...
        s = scatter(p1,p2,[],csel,'filled','MarkerFaceAlpha',0.5);
