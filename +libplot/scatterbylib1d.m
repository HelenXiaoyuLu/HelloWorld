...
function [libTable,ng,f1,c1,d1,s1] = scatterbylib1d(libTable, pp1,varargin)
    %% parse input
    c = colormap(lines);
    p = inputParser;
    ...
    switch doNorm
        case 0
            [libsel,ctrlsel,f1,c1,d1,s1] = doScatter(libTable,pp1,controls,selectLib,figshow,cmap,norm2ctrl,datatip);
        case 1
            ...
            [libsel,ctrlsel,f1,c1,d1,s1] = doScatter(libTable,newpp1,controls,selectLib,figshow,cmap,norm2ctrl,datatip); 
    end
    ng =  [libsel,ctrlsel];
end

function [libsel,ctrlsel,f,c,d,s] = doScatter(libTable,pp1,controls,selectLib,figshow,cmap,norm2ctrl,datatip)
...
%         s.DataTipTemplate.DataTipRows(end+1) = row;
    ...
    for c = 1:length(ctrlsel)
        ...
        p2 = p2end+ones(size(p1))*c+0.5*rand(size(p1))-0.25;
