...
%}
%%
function[h,L,MX,MED,bw]=dualViolin(Y1,Y2,varargin)
...
for i=1:size(Y2,2)    
    if isempty(b)==0
        [f, u, bb]=ksdensity(Y2{i},'bandwidth',b(i));
    elseif isempty(b)
        [f, u, bb]=ksdensity(Y2{i});
    end
...
for i=i:size(Y1,2)
    ...
    if setX == 0
        if plotmean == 1
            p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i*ones(size(F(:,i)))),MX(:,i)) ],[MX(:,i) MX(:,i)],mc,'LineWidth',1);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i)), interp1(flipud(U(:,i)),flipud(i*ones(size(F(:,i)))),MED(:,i)) ],[MED(:,i) MED(:,i)],medc,'LineWidth',1);
        end
    elseif setX == 1
        if plotmean == 1
...
for i=i:size(Y2,2)
    if isempty(lc) == 1
        if setX == 0
            h(i)=fill([i*ones(size(F2(:,i)));flipud(i-F2(:,i))],[U2(:,i);flipud(U2(:,i))],fc(i,:),'FaceAlpha',alp+0.2,'EdgeColor','none');
        else
            h(i)=fill([x(i)*ones(size(F2(:,i)));flipud(x(i)-F2(:,i))],[U2(:,i);flipud(U2(:,i))],fc(i,:),'FaceAlpha',alp+0.2,'EdgeColor','none');
        end
    else
        if setX == 0
            h(i)=fill([i*ones(size(F2(:,i)));flipud(i-F2(:,i))],[U2(:,i);flipud(U2(:,i))],fc(i,:),'FaceAlpha',alp+0.2,'EdgeColor',lc);
        else
            h(i)=fill([x(i)*ones(size(F2(:,i)));flipud(x(i)-F2(:,i))],[U2(:,i);flipud(U2(:,i))],fc(i,:),'FaceAlpha',alp+0.2,'EdgeColor',lc);
        end
    end
    hold on
    if setX == 0
        if plotmean == 1
            p(1)=plot([interp1(U2(:,i),i*ones(size(F2(:,i))),MX2(:,i)), interp1(flipud(U2(:,i)),flipud(i-F2(:,i)),MX2(:,i)) ],[MX2(:,i) MX2(:,i)],mc2,'LineWidth',1);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U2(:,i),i*ones(size(F2(:,i))),MED2(:,i)), interp1(flipud(U2(:,i)),flipud(i-F2(:,i)),MED2(:,i)) ],[MED2(:,i) MED2(:,i)],medc2,'LineWidth',1);
        end
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([interp1(U2(:,i),F2(:,i)+i,MX2(:,i))+x(i)-i, interp1(flipud(U2(:,i)),flipud(i-F2(:,i)),MX2(:,i))+x(i)-i],[MX2(:,i) MX2(:,i)],mc2,'LineWidth',1);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U2(:,i),F2(:,i)+i,MED2(:,i))+x(i)-i, interp1(flipud(U2(:,i)),flipud(i-F2(:,i)),MED2(:,i))+x(i)-i],[MED2(:,i) MED2(:,i)],medc2,'LineWidth',1);
        end
    end
end
...
%% Set axis
if setX == 0
    axis([0.5 size(Y1,2)+0.5, min(U(:)) max(U(:))]);
elseif setX == 1
