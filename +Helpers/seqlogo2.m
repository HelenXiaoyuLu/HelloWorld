function [W, hfig] = seqlogo(P, varargin)
...
%   is log2(4) bits for DNA/RNA and log2(20) bits for proteins. SEQS is a
...
%       6097-6100, 1990. 
% 
%   See also HMMPROFDEMO, SEQCONSENSUS, SEQDISP, SEQPROFILE.
...
if nargin > 1
    ...
    okargs = {'alphabet','startat','endat','displaylogo','tofile', 'sscorrection','ActualSt'};
    for j=1:2:nargin-2
        ...
        switch(k)
            ...
            case 7 % error correction
                ActualPos = pval;
                if isempty(ActualPos)
                        ActualPos = NaN;
                end
        end % end of switch
    end
...
if displayLogo
    ...
    if ~isempty(fileName)  % Save a image file
        hfig = seqshowlogo(wtM, symbolList, isAA, startPos, fileName);
    elseif ~isnan(ActualPos)
        hfig = seqshowlogo(wtM, symbolList, isAA, ActualPos);
    else        
